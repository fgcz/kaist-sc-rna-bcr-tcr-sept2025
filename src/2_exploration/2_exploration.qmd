...
Cell cycle information can be useful to keep in mind when interpreting downstream analyses.
...
There are some annotations that contain only a few cells. They are usually not of interest, and they clog our plots. Therefore we remove them from the annotation:
...
### Comparing with the automated 10x cell annotation

You might have noticed that Donor1 actually came with a cell annotation file already present: `~/work/5k-human-pbmc-3p-gem-x/Donor1/cell_types` .

Let's load this result and compare it against the celltype annotation we got from SingleR

```{r}
# read in the celltypes
cellTypes <- vroom::vroom("~/work/5k-human-pbmc-3p-gem-x/Donor1/cell_types/cell_types.csv", delim=",")

# create a named vector mapping cells to labels
cellTypes$barcode <- paste0("Donor1_", cellTypes$barcode)
cellTypeLabel <- cellTypes$coarse_cell_type
names(cellTypeLabel) <- cellTypes$barcode

# Create a named vector for all unlabeled cells in our dataset
cellTypeUnlabeledCells <- setdiff(colnames(scData), cellTypes$barcode)
cellTypeUnlabeled <- rep(NA, length(cellTypeUnlabeledCells))
names(cellTypeUnlabeled) <- cellTypeUnlabeledCells

# Merge
cellTypeLabel <- c(cellTypeLabel, cellTypeUnlabeled)

scData <- AddMetaData(scData, cellTypeLabel, col.name="TenX_annot")
```

And repeat the visualisation on our reduced dimensionality

```{r}
dittoSeq::dittoDimPlot(scData, "TenX_annot", size = 0.7,
                       cells.use = !is.na(scData$TenX_annot),
                       show.others = TRUE,
                       opacity=0.8)
```
...
It can take quite some time on limited computational setups (1.5 minutes per cluster).
:::

Now run analysis.

```{r}
# Here we load the result if we have saved it before to save time
de_genes_fn <- "de_genes_FindAllMarkers.csv"
if (file.exists(de_genes_fn)) {
  de_genes <- vroom::vroom(de_genes_fn, delim="\t")
} else {
  de_genes <- FindAllMarkers(scData,  min.pct = 0.25, only.pos = TRUE)
}

```

Subset the table to only keep the significant genes, and you can save it as a csv file if you wish to explore it further. Then extract the top 5 markers per cluster:

```{r}
de_genes <- subset(de_genes, de_genes$p_val_adj < 0.05)
vroom::vroom_write(de_genes,
...
```{r}
dittoSeq::dittoDotPlot(scData,
                       vars = unique(top_specific_markers$gene), 
                       group.by = "RNA_snn_res.0.4")
```

Are the T cell genes in there? You can re-load the vector with immune genes and plot them.
...
CCA is the original integration method implemented in Seurat, and which we used above, but it can be slow for large datasets. In addition, it has a tendency to over-correct, meaning it might obscure biological variation in the data beyond simply correcting for the technical noise we actually want to get rid of.

In the case of this data, we are dealing with PBMCs from two donors, and as such we expect the same cell types in both samples. CCA is expected to perform well in this setting. If however we were dealing with samples which have very different cell type compositions, it might be a good idea to try different methods.

One alternative that has become quite popular in recent years is [Harmony](https://portals.broadinstitute.org/harmony/). It works very well for large datasets and produces good results. Though it can also suffer from overcorrection, it is generally less severe than CCA. Let's try using it now.

```{r}
library(harmony)

scData <- RunHarmony(scData, group.by.vars = "orig.ident", reduction.use="pca")
scData <- FindNeighbors(scData, graph.name="harmonynn", reduction = "harmony")
scData <- FindClusters(scData, graph.name="harmonynn", resolution = seq(0.2, 1.0, by=0.2))
scData <- RunUMAP(scData, reduction = 'harmony', dims=1:30, reduction.name = 'umap.harmony')
```

::: callout-important
### Trying out different integration methods

In recent years, there has been increased emphasis on trying multiple different integration methods, rather than just one, as part of standard single cell analysis. This is because integration method results can vary wildly and choosing the results of only one blindly can bias the results. We can use [certain metrics](https://github.com/carmonalab/scIntegrationMetrics) to determine the level of batch mixing versus celltype mixing to determine which of the integration methods we try looks "the best".

![SC Integration Metrics](report_assets/integration_metrics.jpg)
:::
...
You may notice 2 additional dimensional reductions, `harmony` as well as `umap.harmony` . The first is the harmony reduction which we got when we ran `RunHarmony`. The latter we got when we executed `RunUMAP` , providing the harmony reduction as input.

Additionally, you may notice that we have added another set of cluster assignments based on the harmony reduction, which are stored in metadata variables starting with `harmony_res.`\*
...
Let's try repeating the steps from "Clustering" and "Cell Annotation" again, but this time using the Harmony integration.

```{r}
# your code here
# Hint: You can copy-and-paste the code starting from the "Clustree" section 
# and simply change the `reduction` argument in the function call to DimPlot
# as well as the `prefix` argument to the clustree function call
```
