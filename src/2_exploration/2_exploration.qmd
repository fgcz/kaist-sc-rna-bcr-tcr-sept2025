...
scData <- RunHarmony(scData, group.by.vars = "orig.ident", reduction="pca")
scData <- FindNeighbors(scData, return.neighbor = T, graph.name = 'harmonynn', reduction = "harmony")
scData <- FindClusters(scData, graph.name = 'harmonygraph', resolution = resolution = seq(0.2, 1.0, by=0.2)) 
scData <- RunUMAP(scData, nn.name = 'harmonynn', reduction.name = 'umap.harmony')
```

::: callout-important
## Trying out multiple integration methods
...
![SC Integration Metrics](report_assets/integration_metrics.jpg){alt="SC Integration Metrics"}
:::
